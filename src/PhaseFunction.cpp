...
void fillHenyeyGreenstein(std::vector<std::vector<double>>& moments,
                          int nmom_nstr, int num_layers, double g, int lc) 
{
  const int lo = (lc < 0) ? 0 : lc;
  const int hi = (lc < 0) ? num_layers - 1 : lc;
...
void fillIsotropic(std::vector<std::vector<double>>& moments,
                   int nmom_nstr, int num_layers, int lc) 
{
  const int lo = (lc < 0) ? 0 : lc;
  const int hi = (lc < 0) ? num_layers - 1 : lc;
  for (int layer = lo; layer <= hi; ++layer) {
    for (int k = 0; k <= nmom_nstr; ++k) {
      moments[layer][k] = (k == 0) ? 1.0 : 0.0;
    }
  }
}
                  ...
                  int nmom_nstr, int num_layers, int lc) 
{
  ...
  for (int layer = lo; layer <= hi; ++layer) {
    for (int k = 0; k <= nmom_nstr; ++k) {
      moments[layer][k] = (k == 0) ? 1.0 : (k == 2) ? 0.1 : 0.0;
    }
  }
}
...
void fillHazeGarciaSiewert(std::vector<std::vector<double>>& moments,
                           int nmom_nstr, int num_layers, int lc) 
{
  // Haze-L phase function expansion coefficients (Garcia & Siewert 1985)
  // hazelm[k] is the coefficient for moment k+1 (k=0..81 -> moments 1..82)
  // Stored as: phaseFunctionMoments(k) = hazelm[k-1] / (2k+1)  for k=1..min(82,nmom_nstr)
  ...
  for (int layer = lo; layer <= hi; ++layer) {
    moments[layer][0] = 1.0;
    for (int k = 1; k <= nmom_nstr; ++k) {
      moments[layer][k] = (k <= 82) ? hazelm[k - 1] / (2 * k + 1) : 0.0;
    }
  }
}
...
void fillCloudGarciaSiewert(std::vector<std::vector<double>>& moments,
                            int nmom_nstr, int num_layers, int lc) 
{
  // Cloud C.1 phase function expansion coefficients (Garcia & Siewert 1985)
  // cldmom[k] is the coefficient for moment k+1 (k=0..297 -> moments 1..298)
  // Stored as: phaseFunctionMoments(k) = cldmom[k-1] / (2k+1)  for k=1..min(298,nmom_nstr)
  ...
  for (int layer = lo; layer <= hi; ++layer) {
    moments[layer][0] = 1.0;
    for (int k = 1; k <= nmom_nstr; ++k) {
      moments[layer][k] = (k <= 298) ? cldmom[k - 1] / (2 * k + 1) : 0.0;
    }
  }
}
...
void fillPhaseFunction(std::vector<std::vector<double>>& moments,
                       ...
                       PhaseFunction type, double g, int lc) 
{
  switch (type) {
    case PhaseFunction::Isotropic:         fillIsotropic(moments, nmom_nstr, num_layers, lc);           break;
    case PhaseFunction::Rayleigh:          fillRayleigh(moments, nmom_nstr, num_layers, lc);            break;
    case PhaseFunction::HenyeyGreenstein:  fillHenyeyGreenstein(moments, nmom_nstr, num_layers, g, lc); break;
    case PhaseFunction::HazeGarciaSiewert: fillHazeGarciaSiewert(moments, nmom_nstr, num_layers, lc);   break;
    case PhaseFunction::CloudGarciaSiewert:fillCloudGarciaSiewert(moments, nmom_nstr, num_layers, lc);  break;
  }
}
